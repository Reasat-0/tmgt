...
import { HTTP_BAD_REQUEST, HTTP_INTERNAL_SERVER_ERROR, HTTP_OK, } from '../utils/httpStatusCodes.js';
...
export const register = async (req, res) => {
    try {
        ...
        res.status(201).json(res.status(HTTP_OK).json(generateResponse({
...
export const login = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ id: user.id, email: user.email }, process.env.JWT_SECRET, { expiresIn: '1d' } // Token expires in 1 day
        );
        // Sending token and user info in response
        res.status(HTTP_OK).json(generateResponse({
